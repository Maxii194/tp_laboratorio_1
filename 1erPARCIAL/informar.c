...
int informar_rubroConMasPublicaciones(Aviso* arrayAvisos,int limiteAvisos){
	...
	int idAvisoConMasPublicaciones;
	...
	for(j=0;j<limiteAvisos;j++){
		...
		for(i=0;i<limiteAvisos;i++)
		{
			if(arrayAvisos[j].idAviso == arrayAvisos[i].idAviso && arrayAvisos[i].estado == 1){
				...
				idAvisoConMasPublicaciones = j;
			}
		}	
	}
	
	printf("El rubro con mas publicaciones Activas es: Rubro %d",arrayAvisos[idAvisoConMasPublicaciones].numeroRubro);
